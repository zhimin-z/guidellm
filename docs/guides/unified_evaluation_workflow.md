...
- GuideLLM does not support loading model weights directly into memory
- All inference is performed via HTTP API calls to external servers
- Note: While the backend servers (like vLLM) load models locally, GuideLLM itself only interacts via API
...
### Supported Strategies: 13 out of 50
...
**Fully Supported (✅):**
1. Git Clone installation
2. PyPI package installation
3. Container image deployment
4. Model API authentication (OpenAI-compatible endpoints)
5. Artifact repository authentication (Hugging Face)
6. Remote inference (Model-as-a-Service)
7. Offline benchmark data preparation
8. Synthetic data generation
9. Batch inference with multiple execution profiles
10. Performance measurement (latency, throughput, token metrics)
11. Distributional statistics
12. Chart generation (HTML reports)
...
- Local model inference (API-only)
